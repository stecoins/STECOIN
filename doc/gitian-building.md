...
*Setup instructions for a gitian build of SATELLITE using a Debian VM or physical system.*
...
- [Building SATELLITE](#building-satellite)
...
Clone the git repositories for satellite and gitian and then checkout the satellite version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the satellite repository
...
To build SATELLITE (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the satellite repository.
...
```bash
    ...
    From https://github.com/satellite/satellite
...
For example:
```bash
URL=https://github.com/konez2k/satellite.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/satellite-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/satellite-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/satellite-build.assert
```
...
[satellite/gitian.sigs](https://github.com/satellite/gitian.sigs/) repository, or if that's not possible to create a pull
request.
